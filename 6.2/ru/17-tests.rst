...
Как показано ранее, CLI-команды Symfony автоматически подставляют переменную окружения ``DATABASE_URL``. Когда значение ``APP_ENV`` равно ``test`` (например, как при выполнении тестов PHPUnit), Symfony изменит имя базы данных с ``main`` на ``main_test``, создав отдельную базу данных для тестов.
