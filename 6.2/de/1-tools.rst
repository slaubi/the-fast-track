...
Verwende PHP 8.1 und überprüfe, ob die folgenden PHP-Erweiterungen installiert sind oder installiere sie jetzt: ``intl``, ``pdo_pgsql``, ``xsl``, ``amqp``, ``gd``, ``openssl``, ``sodium``. Optional installiere auch ``redis``, ``curl`` and  ``zip``.
