...
Useremo PHP 8.3, controllate che queste estensioni PHP siano installate: ``intl``, ``pdo_pgsql``, ``xsl``, ``amqp``, ``gd``, ``openssl``, ``sodium``. Le estensioni ``redis``, ``curl``, e ``zip`` sono consigliate, ma opzionali.
