...
PHPは8.3バージョンを使い、下記の PHP 拡張` がインストール済みになっていることを確認してください。もし入っていなければ今インストールしてください： ``intl``, ``pdo_pgsql``, ``xsl``, ``amqp``, ``gd``, ``openssl``, ``sodium`` 。オプションで下記をインストールしてください： ``redis``, ``curl`` , ``zip`` 。
