...
Comme le livre doit sortir pendant la SymfonyCon d'Amsterdam, ce serait intéressant si le projet était en quelque sorte relié à Symfony et aux conférences. Et pourquoi pas un `livre d'or`_ ? J'aime le côté démodé et désuet de développer un livre d'or au 21ème siècle !
...
* 550 lignes logiques de code (LLOC) de PHP, tel que rapporté par `PHPLOC`_ ;

* 40 lignes de configuration personnalisée réparties dans 3 fichiers (via attributs et YAML), principalement pour configurer le design de l'interface d'administration ;
...
Clonez le `dépôt du livre d'or`_ quelque part sur votre machine :
...
.. _`dépôt du livre d'or`: https://github.com/the-fast-track/book-5.0-1
