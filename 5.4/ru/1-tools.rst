...
Используйте PHP 8.1 и установите следующие :index:`следующие модули PHP <PHP-модули>`, если они отсутствуют: ``intl``, ``pdo_pgsql``, ``xsl``, ``gd``, ``openssl``, ``sodium``. Также можно установить ``redis``, ``curl`` и ``zip``.
