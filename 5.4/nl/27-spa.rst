...
De ``API_ENDPOINT``-omgevingsvariabele moet verwijzen naar de webserver van de website waar het API-endpoint ``/api`` staat. We zullen dit binnenkort beter configureren wanneer we ``yarn`` draaien.
