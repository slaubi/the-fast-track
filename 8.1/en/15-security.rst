...
Even though attendees won't be able to create their own accounts on the website, we are going to create a fully functional authentication system for the admin. We will therefore only have one user, the website admin.
...
Choose whatever you like as a password and run the following command to generate the password hash:
...
Run the ``make:security:form-login`` command to update the security configuration, generate a login template, and create an *authenticator*:
...
Name the controller ``SecurityController`` and generate a ``/logout`` URL (``yes``).
