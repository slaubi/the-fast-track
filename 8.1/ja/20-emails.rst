...
次に、HTTP リクエスト外で URL を生成する際のデフォルト URI として使用するよう、ルーターに指定します:
...
``SYMFONY_DEFAULT_ROUTE_URL`` の環境変数は、``symfony`` CLI では自動的にローカルに設定されます。また、Upsun の際は設定をベースに指定されます。
...
Upsun では、特別な設定は必要ありません。全てのアカウントは、SendGrid のアカウントが付いてくるので、メール送信の際には、自動的にそのアカウントが使用されます。
...
.. _`Upsun でのメールに関するドキュメント`: https://symfony.com/doc/current/cloud/services/emails.html
