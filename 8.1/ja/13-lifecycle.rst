...
最後に行う変更として、コントローラーやテンプレートでカンファレンスの ``id`` の代わりに ``slug`` をルートで使用するように修正しましょう。ルートパラメータがエンティティのプライマリーキーではなくなったため、``{slug:conference}`` という構文を使って、``slug`` プロパティで照合して ``$conference`` を取得するよう Symfony に指示します。``#[MapEntity]`` 属性はもう必要ありません:
