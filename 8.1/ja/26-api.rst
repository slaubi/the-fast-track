...
デフォルトでは、 ``.env`` で定義されている ``localhost`` と ``127.0.0.1`` の任意ポートからのHTTPリクエストが許可されています。モバイルアプリケーションや外部のフロントエンドなど、別のドメインでホストされているアプリケーションが API を呼び出す必要がある場合は、この値を調整してください。
