...
LLM がコードを書いてくれる今、書籍を読むことに時間を費やす価値はまだあるのでしょうか？この版に取り組みながら、私自身この問いを考えました。私の答えは、間違いなく「はい」です。私は毎日 AI ツールを使っていて、今では私のコードのほとんどすべてを AI が書いています。しかし、本当に大事なことは変わっていません。アプリケーションのアーキテクチャを理解し、何を作るかを決め、生成されたコードをレビューし、わずかに間違っている箇所に気づく必要があります。理解できないコードはレビューできませんし、思い描けないアーキテクチャへとツールを導くこともできません。書籍を読む意義は、判断力にあるのであって、ノスタルジーではありません。

判断力には理解が必要です。そして、理解こそがこの書籍が築き上げるものです。最初のコミットから本番環境まで、Symfony アプリケーションの完全なメンタルモデルを手に入れられます。このモデルが頭の中にあれば、次に LLM がコントローラーや Messenger のハンドラーを生成したときに、それが正しいかどうか、そしてなぜ正しいのかが分かるようになります。さらに、コメントをモデレートするために、アプリケーションの中で実際に LLM を働かせてみます。
...
最後ですが、大事なことがあります。もし、はまってしまったらヘルプを求めることに躊躇しないでください。エッジケースに当たってしまった可能性もありますし、自分では見つけて修正するのに困難なタイポをしこんでしまっているかもしれません。質問をぜひしてください。 `Slack`_ や `GitHub`_ に素晴らしいコミュニティがありますので、ぜひ使ってください。
