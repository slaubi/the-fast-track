...
Для управління сервісами на нашому локальному комп'ютері ми вирішили використовувати Docker. Згенерований файл ``compose.yaml`` вже містить PostgreSQL як сервіс:
...
Додавання PostgreSQL до Upsun
----------------------------------------------

.. index::
    single: Upsun;PostgreSQL

Для продакшн-інфраструктури у Upsun додавання такого сервісу, як PostgreSQL, виконується у файлі ``.upsun/config.yaml``, що вже було зроблено за допомогою рецепту пакету ``webapp``:
...
Сервіс ``database`` є базою даних PostgreSQL (та ж версія, що і для Docker). Upsun автоматично виділяє диск під час першого розгортання; за потреби налаштуйте його пізніше за допомогою ``symfony cloud:resources:set``.

Нам також потрібно "зв'язати" БД з контейнером застосунку, який описаний у ``.upsun/config.yaml``:
...
Доступ до бази даних у Upsun
----------------------------------------------------

PostgreSQL тепер працює як локально, за допомогою Docker, так і в продакшн, у Upsun.
...
Якщо ви хочете під'єднатися до PostgreSQL, розміщеному у продакшн контейнері, ви можете відкрити тунель SSH між локальним комп'ютером і інфраструктурою Upsun:
...
За замовчуванням сервіси Upsun не відображаються як змінні середовища на локальному комп'ютері. Ви маєте налаштувати це явно, виконавши команду ``var:expose-from-tunnel``. Чому? Підключення до до бази даних у продакшн є небезпечною операцією. Ви можете навести безлад у *реальних* даних.
...
Docker Compose й Upsun легко працюють із Symfony завдяки змінним середовища.
...
Коли тунель до Upsun відкрито за допомогою команди ``var:expose-from-tunnel``, команда ``var:export`` повертає змінні віддаленого середовища:
...
.. _`Сервіси Upsun`: https://symfony.com/doc/current/cloud/services/intro.html#available-services
.. _`Тунель Upsun`: https://symfony.com/doc/current/cloud/services/intro.html#connecting-to-a-service
.. _`Документація по PostgreSQL`: https://www.postgresql.org/docs/
.. _`Команди Docker Compose`: https://docs.docker.com/reference/cli/docker/compose/
