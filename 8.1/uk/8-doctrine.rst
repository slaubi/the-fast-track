...
Замість того щоб жорстко задати значення ``DATABASE_URL`` у файлі, ми можемо додати префікс ``symfony`` до всіх команд. Це дозволить виявити сервіси запущені у Docker і/чи Upsun (коли відкрито тунель) і автоматично встановити змінну середовища.

Docker Compose й Upsun легко працюють із Symfony завдяки цим змінним середовища.
...
Пам'ятаєте *ім'я сервісу* ``database``, що використовується у конфігураціях Docker і Upsun? Імена сервісів використовуються як префікси для визначення змінних середовища, таких як ``DATABASE_URL``. Якщо ваші сервіси іменуються відповідно до домовленостей Symfony, то додаткові налаштування не потрібні.
...
Під час розгортання проекту Upsun оновлює код, але також виконує міграцію бази даних, якщо така є (він виявляє, чи існує команда ``doctrine:migrations:migrate``).
