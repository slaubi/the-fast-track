...
Остання зміна полягає в оновленні контролерів і шаблонів для використання ``slug`` замість ``id`` конференції для маршрутів. Оскільки параметр маршруту більше не є первинним ключем сутності, вкажіть атрибуту ``#[MapEntity]``, яку властивість зіставляти, передавши явне ``mapping``:
