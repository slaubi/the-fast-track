...
Остання зміна полягає в оновленні контролерів і шаблонів для використання ``slug`` замість ``id`` конференції для маршрутів. Оскільки параметр маршруту більше не є первинним ключем сутності, використайте синтаксис ``{slug:conference}``, щоб вказати Symfony отримувати ``$conference`` за збігом її властивості ``slug``; атрибут ``#[MapEntity]`` більше не потрібен:
