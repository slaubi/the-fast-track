...
Обратите внимание, что объект ``Request`` теперь внедряется в контроллер, поскольку он нужен форме для проверки отправленных данных через ``handleRequest()``.
...
На последнем шаге мы рассмотрим возможность хранения загруженных файлов на продакшен-серверах. Зачем нам нужно что-то делать для этого? Всё из-за того, что большинство современных облачных платформ по ряду причин используют контейнеры только для чтения. Upsun — не исключение.
...
Откройте файл ``.upsun/config.yaml``. В нём уже есть точка *монтирования* с возможностью записи, указанная для директории ``var/``. Директория ``var/`` — единственное место в файловой системе, в которую Symfony может что-то записывать (кеш, логи и т.п.).
