...
Данный метод работает несколько иначе, чем остальные, которые мы встречали ранее. Мы указываем, что экземпляр ``Conference`` должен быть внедрён в метод. Однако в базе данных может быть несколько конференций, и атрибут ``#[MapEntity]`` указывает Symfony получить нужную по идентификатору ``{id}``, переданному в пути запроса (``id`` — это первичный ключ в таблице ``conference`` базы данных).
...
Атрибут ``#[MapQueryParameter]`` сопоставляет параметр ``offset`` из строки запроса с аргументом контроллера ``$offset``; если параметр не задан, значение по умолчанию равно ``0``. Поскольку смещение приходит от клиента, параметр ``min_range`` проверяет, что оно не отрицательное; Symfony возвращает ответ 404, если значение недопустимо.
