...
Осталось сделать последнее изменение — заменить в контроллерах и шаблонах параметр маршрутов конференций с ``id`` на ``slug``. Поскольку параметр маршрута больше не является первичным ключом сущности, используйте синтаксис ``{slug:conference}``, чтобы Symfony получала ``$conference``, сопоставляя её свойство ``slug``; атрибут ``#[MapEntity]`` больше не нужен:
