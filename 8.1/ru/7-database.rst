...
Для управления сервисами на локальном компьютере будем использовать Docker. Сгенерированный файл ``compose.yaml`` уже содержит определение сервиса для PostgreSQL:
...
.. note::
    ...
    Если на вашем локальном компьютере не установлена команда ``psql``, вы можете запустить её через ``docker compose``:
...
Добавление PostgreSQL в Upsun
----------------------------------------------

.. index::
    single: Upsun;PostgreSQL

Добавление такого сервиса, как PostgreSQL, в инфраструктуру продакшена на Upsun, делается через изменения в файле ``.upsun/config.yaml``, что уже было сделано с помощью рецепта пакета ``webapp``:
...
Сервис ``database`` — это PostgreSQL (такой же версии, что и для Docker). Upsun выделяет диск автоматически при первом развёртывании; при необходимости его можно изменить позже с помощью ``symfony cloud:resources:set``.

Также необходимо "привязать" БД к контейнеру приложения, который описан в ``.upsun/config.yaml``:
...
Доступ к базе данных на Upsun
------------------------------------------------------

PostgreSQL теперь работает как локально через Docker, так и на продакшене в Upsun.
...
Если вы хотите подключиться к PostgreSQL, расположенном в контейнерах на продакшене, вы можете открыть SSH-туннель между вашей локальной машиной и инфраструктурой Upsun:
...
По умолчанию сервисы Upsun не отображаются в переменных окружения на локальной машине. Нужно указать это явно, используя флаг ``--expose-env-vars``. Почему? Подключение к базе данных на продакшен-сервере довольно опасно, поскольку есть риск повредить *реальные* данные.
...
Docker Compose и Upsun отлично работают с Symfony благодаря переменным окружения.
...
Когда туннель к Upsun открыт с помощью ``var:expose-from-tunnel``, команда ``var:export`` возвращает переменные из удалённого окружения:
...
Возможно, вы ещё этого не осознали, но хранение инфраструктуры в файлах вместе с кодом очень помогает в работе. Настройка инфраструктуры проекта для Docker и Upsun производится в конфигурационных файлах. Когда новой функциональности потребуется дополнительный сервис, изменения в коде и инфраструктуре будут зафиксированы в одном патче.
...
.. sidebar:: Двигаемся дальше
    ...
    * `команды Docker Compose`_.

.. _`Сервисы Upsun`: https://symfony.com/doc/current/cloud/services/intro.html#available-services
.. _`Туннель Upsun`: https://symfony.com/doc/current/cloud/services/intro.html#connecting-to-a-service
.. _`Документация PostgreSQL`: https://www.postgresql.org/docs/
.. _`команды Docker Compose`: https://docs.docker.com/reference/cli/docker/compose/
