...
По умолчанию это значение определено в файле ``.env`` и разрешает выполнять HTTP-запросы с ``localhost`` и ``127.0.0.1`` через любой порт. Измените её, когда приложению, размещённому на другом домене, например мобильному приложению или внешнему фронтенду, потребуется обращаться к API.
