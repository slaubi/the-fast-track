...
attribute‌ی ``#[MapQueryParameter]`` پارامتر رشته‌ی پرس‌وجوی ``offset`` را به آرگمان ``$offset`` کنترلر می‌نگارد و در صورت تنظیم‌نشدن، مقدار پیشفرض را برابر با ``0`` قرار می‌دهد. از آنجایی که offset از سمت کلاینت می‌آید، گزینه‌ی ``min_range`` اعتبارسنجی می‌کند که منفی نباشد؛ سیمفونی در صورت نامعتبر بودن مقدار، یک پاسخ ۴۰۴ بازمی‌گرداند.
