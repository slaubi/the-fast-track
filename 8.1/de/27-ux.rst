...
Upsun ermöglicht es, mehrere Anwendungen pro Projekt zu deployen. Wechsle zurück ins Projekt-Stammverzeichnis und füge ``.upsun/config.yaml`` eine zweite Anwendung namens ``spa`` hinzu, deren Wurzel das Verzeichnis ``spa/`` ist:
...
Bearbeite die ``.upsun/config.yaml``-Datei, um die ``spa.``-Subdomain an die ``spa``-Anwendung weiterzuleiten:
...
Wenn Deine Domain ``https://master-5szvwec-hzhac461b3a6o.eu-5.platformsh.site/`` ist, wird sie durch die``sed``-Aufrufe zu  ``https://spa.master-5szvwec-hzhac461b3a6o.eu-5.platformsh.site`` umgewandelt.
...
.. sidebar:: Weiterführendes
    ...
    * `Die offizielle Preact Website`_;

    * `Die offizielle Cordova-Website`_.

.. _`Preact`: https://preactjs.com/
.. _`Die offizielle Preact Website`: https://preactjs.com/
.. _`Die offizielle Cordova-Website`: https://cordova.apache.org/
