...
.. warning::
    ...
    Gebruik nooit ``docker-compose down`` als je geen data wilt verliezen, tenzij je eerst een back-up maakt.
...
De ``db`` service is een PostgreSQL database (dezelfde versie als we in Docker gebruiken) die we willen opzetten in een kleine container met 1GB schijfruimte.
...
Je hebt het misschien nog niet gerealiseerd, maar het helpt veel om de infrastructuur naast de code in bestanden op te slaan. Docker en SymfonyCloud gebruiken configuratiebestanden om de project-infrastructuur te omschrijven. Wanneer een nieuwe functie een aanvullende service nodig heeft, veranderen de code en maken de wijzigingen in de infrastructuur deel uit van dezelfde patch.

.. sidebar:: Verder gaan

    * `SymfonyCloud diensten <https://symfony.com/doc/current/cloud/services/intro.html#available-services>`_;
