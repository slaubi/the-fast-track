...
Gebruik PHP 8.0 en controleer of de :index:`volgende PHP extensions <PHP extensions>` geïnstalleerd zijn, of installeer ze nu: ``intl``, ``pdo_pgsql``, ``xsl``, ``amqp``, ``gd``, ``openssl`` en ``sodium``. Installeer optioneel ook de ``redis``, ``curl`` en ``zip`` extensies.
...
We schrijven weinig JavaScript code, maar we zullen JavaScript/NodeJS tools gebruiken om onze assets te beheren. Controleer dat je `NodeJS`_ en de `Yarn`_ package manager geïnstalleerd hebt.
...
Installeer de `Symfony CLI`_ en verplaats deze naar jouw ``$PATH``. Maak een `SymfonyConnect`_ account  aan als je die nog niet hebt en log in via ``symfony login``.
