...
.. note::
    ...
    Zgodnie z tym, co widziałeś w poprzednim rozdziale, ``APP_ENV=test`` oznacza, że zmienna środowiskowa ``APP_ENV`` jest ustawiona w ramach kontekstu polecenia. Jeżeli używasz Windowsa, użyj ``--env=test`` zamiast ``symfony console secrets:set AKISMET_KEY --env=test``.
...
Widzieliśmy również, że narzędzie Symfony CLI automatycznie udostępnia zmienną środowiskową ``DATABASE_URL``. Kiedy zmienna ``APP_ENV`` jest ustawiona na ``test``, podobnie jak wtedy, kiedy uruchamialiśmy PHPUnit, zmienia nazwę bazy danych z ``main`` na ``main_test``, aby testy miały swoją własną bazę danych. To bardzo istotne, abyśmy mieli stabilne dane, na których możemy uruchamiać testy i nie nadpisywali tych, które przechowujemy w bazie deweloperskiej.
...
Zauważ, że ustawiamy jawnie zmienną ``APP_ENV``, nawet kiedy uruchamiany PHPUnit, aby Symfony CLI mogło zmienić bazę na ``main_test``.
...
.. warning::
    ...
    Wcięcia w regułach pliku Makefile muszą się składać z pojedynczego znaku tabulacji zamiast spacji.
...
Innym sposobem na przeprowadzenie testów funkcjonalnych jest użycie `Blackfire player <https://blackfire.io/player>`_. Oprócz zwykłych testów funkcjonalnych, potrafi on również przeprowadzać testy wydajnościowe.

Aby dowiedzieć się więcej, zapoznaj się z krokiem "Wydajność".

.. sidebar:: Idąc dalej

    * `Lista asercji definiowanych przez Symfony <https://symfony.com/doc/current/testing/functional_tests_assertions.html>`_ dla testów funkcjonalnych;

    * `Dokumentacja PHPUnit <https://phpunit.de/documentation.html>`_

    * `Biblioteka Faker <https://github.com/FakerPHP/Faker>`_ do generowania danych testowych;

    * `Dokumentacja komponentu CssSelector <https://symfony.com/doc/current/components/css_selector.html>`_;

    * `Symfony Panther <https://github.com/symfony/panther>`_  biblioteka do testowania przez przeglądarkę i przeszukiwania (ang. crawling) stron internetowych w aplikacjach opartych na Symfony;

    * `Dokumentacja Make/Makefile <https://www.gnu.org/software/make/manual/make.html>`_.
