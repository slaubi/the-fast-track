...
Также нужно остановить потребитель сообщений, который тоже не еще не знает о перехватчике почты:

.. code-block:: bash

    $ symfony console messenger:stop-workers

И снова запустим. Переменная ``MAILER_DSN`` теперь автоматически задана:
...
В данный момент шина отправляет сообщения электронной почты синхронно, так как мы не настроили очередь. И снова воспользуемся RabbitMQ:
...
.. sidebar:: Двигаемся дальше
    ...
    * `Документация по настройке электронной почты в SymfonyCloud <https://symfony.com/doc/current/cloud/services/emails.html>`_.
