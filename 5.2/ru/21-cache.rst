...
Для тестирования стратегии HTTP-кеширования включите обратный прокси-сервер Symfony:
...
Всегда измеряйте результат от использования кеширования с помощью инструментов профилировщика, например, `Blackfire <https://blackfire.io/>`_.
...
Скорее всего, в реальности вы разрешите очистку кеша только с определённых IP-адресов, как об этом описано в `документации Varnish <https://varnish-cache.org/docs/trunk/users-guide/purging.html>`_.
...
URL-адреса выглядит немного необычно, потому что команда ``env:urls`` возвращает адреса, которые уже заканчиваются на символ ``/``.

.. sidebar:: Двигаемся дальше

    * Глобальная облачная платформа `Cloudflare <https://www.cloudflare.com>`_;

    * `Документация по Varnish <https://varnish-cache.org/docs/index.html>`_;

    * `Спецификация ESI <https://www.w3.org/TR/esi-lang>`_ и `ресурсы по ESI <https://www.akamai.com/us/en/support/esi.jsp>`_;

    * `Модель валидации HTTP-кеширования <https://symfony.com/doc/current/http_cache/validation.html>`_;

    * `HTTP-кеширование в SymfonyCloud <https://symfony.com/doc/current/cloud/cookbooks/cache.html>`_.
