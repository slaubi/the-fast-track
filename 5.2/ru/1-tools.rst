...
В своей последней книге я рекомендовал Subversion в качестве системы управления версиями. Сейчас времена изменились — все, как правило, используют `Git`_.
...
Используйте PHP 8.0 и установите следующие :index:`следующие модули PHP <PHP-модули>`, если они отсутствуют: ``intl``, ``pdo_pgsql``, ``xsl``, ``gd``, ``openssl``, ``sodium``. Также можно установить ``redis``, ``curl`` и ``zip``.
...
Установите `Symfony CLI`_ и не забудьте переместить эту утилиту в одну из директорий, заданных в переменной окружения ``$PATH``. Создайте учётную запись `SymfonyConnect`_ если у вас её ещё нет, и авторизуйтесь с помощью команды ``symfony login``.
