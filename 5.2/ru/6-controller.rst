...
С помощью аннотации ``#[Route('/conference', name: 'conference')]`` метод ``index()`` становится контроллером (его объявление вместе с конфигурацией находится непосредственно над кодом).
