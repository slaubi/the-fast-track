...
Настроим таблицы и триггеры PostgreSQL:

.. code-block:: bash

    $ symfony console make:migration

И запустим миграцию базы данных:
...
Чтобы получать сообщения от PostgreSQL, нам нужно постоянно запускать команду ``messenger:consume``. В SymfonyCloud для этой задачи отведена специальная роль — *воркер*:
