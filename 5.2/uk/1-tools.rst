...
У своїй останній книзі, я рекомендував SVN (Subversion), у якості системи контролю версій. Але, схоже, що зараз всі використовують `Git`_.
...
Використовуйте PHP 8.0 і перевірте чи встановлено :index:`наступні розширення PHP <розширення PHP>`, або встановіть їх зараз: ``intl``, ``pdo_pgsql``, ``xsl``, ``amqp``, ``gd``, ``openssl``, ``sodium``. За бажанням також встановіть ``redis``, ``curl`` і ``zip``.
...
Управління залежностями в проекті з Symfony — дуже важливо. Встановіть останню версію `Composer`_ — інструменту для управління залежностями в PHP.
...
І останнє, але не менш важливе, ми будемо використовувати ``symfony`` CLI для підвищення нашої продуктивності. Від локального веб-сервера, до повної інтеграції з Docker і підтримки SymfonyCloud, це значно заощадить наш час.

Встановіть `Symfony CLI`_ й помістіть його у каталог змінної середовища ``$PATH``. Створіть обліковий запис `SymfonyConnect`_, якщо у вас його ще немає, і увійдіть у систему за допомогою ``symfony login``.
