...
Щоб протестувати стратегію HTTP-кешу, увімкніть зворотний HTTP-проксі Symfony:
...
Компонент Symfony Process дозволяє вам виконати команду й отримати результат (стандартний вивід та вивід помилок); встановіть його:
...
Завжди вимірюйте вплив додавання кешу за допомогою інструменту профілювання, такого як `Blackfire <https://blackfire.io/>`_.
...
Не використовуйте зворотний проксі Symfony в продакшн. Завжди віддавайте перевагу такому зворотному проксі, як Varnish, у вашій інфраструктурі або комерційному CDN.

Додайте Varnish у сервіси SymfonyCloud:
...
У реальному житті ви, ймовірно, обмежилися б IP-адресами, як описано в `документації по Varnish <https://varnish-cache.org/docs/trunk/users-guide/purging.html>`_.
...
URL-адреси виглядають дещо дивно через те, що адреси, повернені ``env:urls``, вже закінчуються символом ``/``.

.. sidebar:: Йдемо далі

    * `Cloudflare <https://www.cloudflare.com>`_, глобальна хмарна платформа;

    * `Документація по HTTP-кешу Varnish <https://varnish-cache.org/docs/index.html>`_;

    * `Специфікація ESI <https://www.w3.org/TR/esi-lang>`_ та `ресурси ESI для розробників <https://www.akamai.com/us/en/support/esi.jsp>`_;

    * `Модель валідації HTTP-кешу <https://symfony.com/doc/current/http_cache/validation.html>`_;

    * `HTTP-Кеш у SymfonyCloud <https://symfony.com/doc/current/cloud/cookbooks/cache.html>`_.
