...
`Тип форми`_ описує *поля форми*, пов'язані з моделлю. Він виконує перетворення даних між представленими даними й властивостями класу моделі. За замовчуванням Symfony використовує метадані з сутності ``Comment`` (наприклад, метадані Doctrine) для визначення конфігурації кожного поля. Наприклад, поле ``text`` відмальовується як ``textarea``, оскільки воно використовує більший стовпчик у базі даних.
...
Ніколи не слід створювати екземпляр типу форми безпосередньо. Замість цього використовуйте метод ``createForm()``. Цей метод є частиною ``AbstractController`` і полегшує створення форм.
...
Деякі поля не можна налаштувати автоматично, наприклад ``photoFilename``. Сутність ``Comment`` має зберегти лише ім'я файлу фотографії, але форма має також подбати про завантаження файлу. Щоб впоратися з цим завданням, ми додали поле з назвою ``photo`` як поле з відключеною опцією ``mapped``: воно не буде зіставлено ні з однією властивістю сутності ``Comment``. Ми будемо керувати ним вручну, щоб реалізувати певну логіку (наприклад, збереження завантаженої фотографії на диску).

Як приклад кастомізації, ми також змінили мітку за замовчуванням, для деяких полів.

Обмеження image працює шляхом перевірки типу mime; Потрібен компонент Mime, щоб змусити його працювати:
...
Форма використовує поле з типом ``email`` для введення електронної пошти й додає атрибут ``required`` для більшості полів. Зверніть увагу, що форма також містить приховане поле ``_token``, що використовується для захисту форми від `CSRF-атак <https://owasp.org/www-community/attacks/csrf>`_.

Але якщо відправка форми проходить без HTML-валідації (за допомогою HTTP-клієнта, який не застосовує правила перевірки, наприклад, cURL), невалідні дані можуть потрапити на сервер.
...
Останнім кроком є зберігання завантажених файлів на продакшн-серверах. Чому необхідно робити щось особливе? Тому що більшість сучасних хмарних платформ використовують контейнери тільки для читання, з різних причин. SymfonyCloud не є винятком.
...
Погляньте на файл ``.symfony.cloud.yaml``, там вже є перезаписувана *точка монтування* для каталогу ``var/``. Каталог ``var/`` — це єдиний каталог, у який Symfony записує дані (кеші, журнали, ...).
...
.. sidebar:: Йдемо далі
    ...
    * `Навчальний посібник SymfonyCasts: форми <https://symfonycasts.com/screencast/symfony-forms>`_;

    * Як `кастомізувати відмальовку форм Symfony в HTML <https://symfony.com/doc/current/form/form_customization.html>`_;

    * `Валідація форм Symfony <https://symfony.com/doc/current/forms.html#validating-forms>`_;

    * `Типи полів форм Symfony <https://symfony.com/doc/current/reference/forms/types.html>`_;

    * `Документація по FlysystemBundle <https://github.com/thephpleague/flysystem-bundle/blob/master/docs/1-getting-started.md>`_, який забезпечує інтеграцію з кількома постачальниками хмарних сховищ, такими як AWS S3, Azure та Google Cloud Storage;

    * `Параметри конфігурації Symfony <https://symfony.com/doc/current/configuration.html#configuration-parameters>`_.

    * `Обмеження (правила) Symfony Validation <https://symfony.com/doc/current/validation.html#basic-constraints>`_;

    * `Шпаргалка по Symfony Form <https://github.com/andreia/symfony-cheat-sheets/blob/master/Symfony2/how_symfony2_forms_works_en.pdf>`_.
