...
.. note::
    ...
    Come visto nei capitoli precedenti, ``APP_ENV=test`` significa che la variabile d'ambiente ``APP_ENV`` è impostata per il contesto del comando. Su Windows, utilizzare invece ``--env=test``: ``symfony console secrets:set AKISMET_KEY --env=test``
...
Vale la pena notare che stiamo impostando la variabile d'ambiente ``APP_ENV`` esplicitamente quando eseguiamo PHPUnit, affinché  Symfony CLI usi ``main_test`` come nome del database.
...
.. sidebar:: Andare oltre
    ...
    * La libreria `Faker <https://github.com/FakerPHP/Faker>`_ per generare dati realistici nelle fixture;
