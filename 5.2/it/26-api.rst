...
Qualche annotazione sulla classe Conference è tutto ciò di cui abbiamo bisogno per configurare l'API:
...
L'annotazione principale ``@ApiResource`` configura l'API per le conferenze. Nella fattispecie, limita le operazioni possibili alla sola ``get`` e configura varie cose, quali ad esempio i campi da visualizzare e come ordinare le conferenze.
...
Lo stesso tipo di annotazioni sono usate per configurare la classe.
...
.. sidebar:: Andare oltre
    ...
    * `Tutorial API Platform su SymfonyCasts <https://symfonycasts.com/screencast/api-platform>`_;
