...
Setup der PostgreSQL und Triggers (Auslöser):

.. code-block:: bash

    $ symfony console make:migration

Und migriere die Datenbank:
...
Um Messages von PostgreSQL zu bearbeiten, müssen wir den ``messenger:consume``-Befehl kontinuierlich ausführen. In der SymfonyCloud ist dies die Rolle eines *Workers*:
