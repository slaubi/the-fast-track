...
Aktiviere den Symfony HTTP-Reverse-Proxy um die HTTP-Cache-Strategie zu testen:
...
Die Symfony Process-Komponente ermöglicht es Dir, einen Befehl auszuführen und das Ergebnis zurückzubekommen (Standard- und Fehlerausgabe); installiere sie:
...
Messe immer die Auswirkungen des Hinzufügens eines Cache mit einem Profiler-Tool wie `Blackfire <https://blackfire.io/>`_.
...
Verwende den Symfony Reverse-Proxy nicht auf dem Produktivsystem. Bevorzuge immer einen Reverse-Proxy wie Varnish auf Deiner Infrastruktur oder ein kommerzielles CDN.

Füge Varnish zu den SymfonyCloud-Diensten hinzu:
...
Im wirklichen Leben würdest du das wahrscheinlich auf bestimmte IPs einschränken, wie in der `Varnish Dokumentation <https://varnish-cache.org/docs/trunk/users-guide/purging.html>`_ beschrieben.
...
Die URLs sehen etwas seltsam aus, da die von ``env:urls`` bereits zurückgegebenen URLs mit ``/`` enden.

.. sidebar:: Weiterführendes

    * `Cloudflare <https://www.cloudflare.com>`_, die globale Cloud-Plattform;

    * `Varnish HTTP Cache Dokumentation <https://varnish-cache.org/docs/index.html>`_;

    * `ESI-Spezifikation <https://www.w3.org/TR/esi-lang>`_ und `ESI-Entwicklerressourcen <https://www.akamai.com/us/en/support/esi.jsp>`_;

    * `HTTP-Cache Validierungsmodell <https://symfony.com/doc/current/http_cache/validation.html>`_;

    * `HTTP-Cache in der SymfonyCloud <https://symfony.com/doc/current/cloud/cookbooks/cache.html>`_.
