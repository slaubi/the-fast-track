...
#. Update the infrastructure on Docker and SymfonyCloud if needed (add the PostgreSQL service); 
...
.. note:: 
    ...
    We don't need steps 3 to 5 here as we are re-using the database as the session storage, but the chapter about using Redis shows how straightforward it is to add, test, and deploy a new service in both Docker and SymfonyCloud. 
...
Note that all SymfonyCloud commands work on the current Git branch. This command opens the deployed URL for the ``sessions-in-db`` branch; the URL will look like ``https://sessions-in-db-xxx.eu.s5y.io/``. 
