...
Upsun では、各プロジェクト毎に複数のアプリケーションをデプロイすることができます。プロジェクトのルートディレクトリに戻り、``spa/`` ディレクトリをルートとする ``spa`` という名前の2つ目のアプリケーションを ``.upsun/config.yaml`` に追加します:
...
``.upsun/config.yaml`` ファイルを編集して、``spa.`` サブドメインを ``spa`` アプリケーションにルーティングしてください:
