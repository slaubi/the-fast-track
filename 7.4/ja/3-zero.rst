...
``--upsun`` オプションを有効にしてプロジェクトを作成したため、``.upsun/config.yaml`` 、 ``.upsun/config.yaml`` 、 ``.upsun/config.yaml`` といった必要な設定ファイルが作られており、Upsunにプロジェクトが初期化されています。
...
.. _`Upsun のドキュメント`: https://docs.platform.sh/guides/symfony.html?utm_source=symfony-cloud-sign-up&utm_medium=backlink&utm_campaign=Symfony-Cloud-sign-up&utm_content=symfony-book
