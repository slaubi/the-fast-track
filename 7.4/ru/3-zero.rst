...
* ``--upsun``: Если вы хотите развернуть проект на Upsun, эта опция автоматически сгенерирует готовую конфигурацию Upsun. Upsun — это предпочтительный и самый простой способ развертывания тестовых, временных и продакшен-окружений Symfony в облаке.
...
Я сделал свой выбор: это будет `Upsun`_. У него есть всё необходимое, а кроме этого он помогает финансировать разработку Symfony.
...
Создайте новый удалённый проект Upsun:
...
* При первом запуске этой команды нужно пройти аутентификацию с помощью учётной записи Upsun, если вы ранее этого не сделали.

* Команда создаст новый проект на Upsun (в течение 30 дней вы можете *бесплатно* разместить первый созданный вами проект).
...
Обратите внимание, что вы не увидите стандартную красивую страницу Symfony на Upsun. Почему? Скоро вы узнаете, что Symfony поддерживает работу с несколькими окружениями, а Upsun автоматически развёрнул код в окружении продакшена.
...
.. tip::
    ...
    Если вы хотите удалить проект на Upsun, воспользуйтесь командой ``cloud:project:delete``.
