...
Данный метод работает несколько иначе, чем остальные, которые мы встречали ранее. Мы указываем, что экземпляр ``Conference`` должен быть внедрён в метод. Однако в базе данных могут быть несколько конференций, и атрибут ``#[MapEntity]`` указывает Symfony получить нужную по идентификатору ``{id}``, переданному в строке запроса (``id`` — это первичный ключ в таблице ``conference`` базы данных).
