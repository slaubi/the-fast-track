...
Осталось сделать последнее изменение — заменить в контроллерах и шаблонах параметр маршрутов конференций с ``id`` на ``slug``. Поскольку параметр маршрута больше не является первичным ключом сущности, укажите атрибуту ``#[MapEntity]`` через явный ``mapping``, какое свойство сопоставлять:
