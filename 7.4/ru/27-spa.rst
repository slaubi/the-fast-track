...
Upsun позволяет развёртывать несколько приложений в рамках одного проекта. Вернитесь в корневую директорию проекта и добавьте в ``.upsun/config.yaml`` второе приложение с именем ``spa``, корнем которого является директория ``spa/``:
...
Отредактируйте файл ``.upsun/config.yaml`` так, чтобы перенаправлять запросы с поддомена ``spa.`` в приложение ``spa``:
