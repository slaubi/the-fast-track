...
Usare PHP 8.5 e verificare che le seguenti estensioni PHP siano installate, o
installarle ora: ``intl``, ``pdo_pgsql``, ``xsl``, ``amqp``, ``gd``,
``openssl``, ``sodium`` e ``iconv``. Facoltativamente, installare anche
``redis``, ``curl`` e ``zip``.
