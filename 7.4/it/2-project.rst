...
Sarebbe bello se il progetto fosse in qualche modo correlato a Symfony e la sua community. Visto che ogni anno organizziamo qualche conferenza online e in presenza, perché non sviluppare un `libro degli ospiti`_? Un "livre d'or" come diciamo noi in Francia. Mi piace la sensazione vecchio stile di sviluppare un libro degli ospiti nel ventunesimo secolo!
...
* 550 righe di codice contenente logica (_LLOC_) come riportato da `PHPLOC`_;

* 40 righe di configurazione in tre file (tramite attributi e YAML), necessarie principalmente alla configurazione del design per il backend;
...
Cloniamo il `repository del libro degli ospiti`_ da qualche parte nella nostra macchina locale:
...
Ancora meglio, il repository non contiene solo la versione finale del codice sul branch ``main``. Lo script esegue ogni azione illustrata nel libro, e fa un _commit_ alla fine di ogni sezione. Crea anche dei tag a ogni passo per facilitare l'esplorazione del codice. Bello, vero?
...
.. _`repository del libro degli ospiti`: https://github.com/the-fast-track/book-5.0-1
