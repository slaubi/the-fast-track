...
Choisissez ce que vous voulez comme mot de passe et exécutez la commande suivante pour générer le hash du mot de passe :
...
Exécutez la commande ``make:security:form-login`` pour mettre à jour la configuration de sécurité, générer un template pour la connexion et créer une classe d'authentification (*authenticator*) :
...
Nommez le contrôleur ``SecurityController`` et créez une URL ``/logout`` (``yes``).
