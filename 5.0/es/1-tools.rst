...
`PostgreSQL`_ va a ser nuestra elección para el motor de base de datos.

`RabbitMQ`_ es el elegido para gestionar las colas de mensajes.
...
Usa PHP 7.3 si puedes, tal vez 7.4 dependiendo de cuándo estés leyendo este libro. Comprueba que las :index:`siguientes extensiones de PHP <PHP extensions>` están instaladas o instálalas ahora: ``intl`` , ``pdo_pgsql`` , ``xsl`` , ``amqp`` , ``gd`` , ``openssl`` y ``sodium``. Opcionalmente también puedes instalar  ``redis`` y ``curl``.
...
Hoy en día, gestionar dependencias lo es todo en un proyecto Symfony. Descarga la última versión de `Composer <https://getcomposer.org/>`_ , la herramienta de gestión de paquetes para PHP.
...
Gestionaremos los servicios mediante Docker y Docker Compose. `Instálalos <https://docs.docker.com/install/>`_ e inicia Docker. Si es la primera vez que lo utilizas, familiarízate con él. Pero no te asustes, lo usaremos de forma muy sencilla. Sin configuraciones sofisticadas ni complejas.
...
Instala la `CLI de Symfony <https://symfony.com/download>`_ y muévelo a algún lugar dentro de tu ``$PATH``. Crea una cuenta `SymfonyConnect <https://connect.symfony.com/>`_ si aún no la tienes e inicia sesión a través de ``symfony login``.

Para utilizar HTTPS localmente, también necesitamos `instalar una autoridad de certificación (CA) <https://symfony.com/doc/current/setup/symfony_server.html#enabling-tls>`_ para habilitar el soporte de TLS. Ejecuta el siguiente comando:
...
Si quieres trabajar de forma aún más elegante, también puedes ejecutar el `proxy de Symfony <https://symfony.com/doc/current/setup/symfony_server.html#setting-up-the-local-proxy>`_ . Es opcional, pero te permite obtener un nombre de dominio local que termina en ``.wip`` para tu proyecto.
