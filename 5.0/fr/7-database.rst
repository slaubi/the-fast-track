...
Un serveur PostgreSQL sera alors installé en version 11 et certaines variables d'environnement, qui contrôlent le nom de la base de données et ses identifiants, seront configurées. Les valeurs n'ont pas vraiment d'importance.
...
Le service ``db`` est une base de données PostgreSQL en version 11 (comme pour Docker) que nous voulons provisionner sur un petit conteneur avec 1 Go d'espace disque.
