...
Pour commencer à traduire le site, nous avons besoin d'installer le composant Translation Symfony :
...
Cette commande génère un fichier de traduction (option ``--force``) pour la locale ``fr`` et le domaine ``messages`` (qui contient tous les messages propres à notre application, comme les erreurs de validation ou de sécurité).
