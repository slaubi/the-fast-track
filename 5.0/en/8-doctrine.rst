...
To deal with the database from PHP, we are going to depend on `Doctrine`_, a set of libraries that help developers manage databases:
...
We will still change the ``.env`` file to setup the default ``DATABASE_DSN`` to use PostgreSQL:
...
.. sidebar:: Going Further
    ...
    * `Databases and Doctrine ORM <https://symfony.com/doc/current/doctrine.html>`_ in Symfony applications;

    * `SymfonyCasts Doctrine tutorial <https://symfonycasts.com/screencast/symfony-doctrine/install>`_;

    * `Working with Doctrine Associations/Relations <https://symfony.com/doc/current/doctrine/associations.html>`_;
