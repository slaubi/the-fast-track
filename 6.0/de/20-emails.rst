...
Die Environment-Variablen ``SYMFONY_DEFAULT_ROUTE_PORT`` und  ``SYMFONY_DEFAULT_ROUTE_HOST`` werden bei Verwendung der ``symfony``-CLI automatisch lokal gesetzt. Bei Platform.sh werden sie anhand der Konfiguration bestimmt.
