...
As we have already seen, the Symfony CLI automatically exposes the ``DATABASE_URL`` environment variable. When ``APP_ENV`` is ``test``, like set when running PHPUnit, the database name changes from ``main`` to ``main_test`` so that tests have their very own database. This is very important as we will need some stable data to run our tests and we certainly don't want to override what we stored in the development database.
