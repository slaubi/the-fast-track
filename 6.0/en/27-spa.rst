...
The ``API_ENDPOINT`` environment variable should point to the web server of the website where we have the API endpoint under ``/api``. We will configure it properly when we will run ``yarn`` soon.
...
Platform.sh allows to deploy multiple applications per project. Adding another application can be done by creating a ``.platform.app.yaml`` file in any sub-directory. Create one under ``spa/`` named ``spa``:
