...
Даже если пакет ``webapp`` автоматически добавил много хороших пакетов, для некоторых специфических функций нам нужно добавить больше зависимостей. Как мы можем добавить больше зависимостей? С помощью Composer. Помимо "обычных" пакетов Composer, мы будем работать с двумя "специальными" видами пакетов:
...
``admin`` — это псевдоним для пакета ``easycorp/easyadmin-bundle``.
...
Ещё одна приятная особенность заключается в том, что вы всегда можете опустить поставщика ``symfony``. Укажите ``cache`` вместо ``symfony/cache``.
